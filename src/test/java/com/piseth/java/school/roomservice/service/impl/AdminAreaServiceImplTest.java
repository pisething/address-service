...
class AdminAreaServiceImplTest {
	...
	class Create {
		...
		void province_success() {
			...
			// duplicate check only (existsById(code))
			...
			// exactly one existsById(String) call, for duplicate check
		...
		void nonProvince_success() {
			...
			when(repo.existsById(eq(parent))).thenReturn(Mono.just(true)); // parent ok
			when(repo.existsById(eq(code))).thenReturn(Mono.just(false)); // no duplicate
			when(repo.save(candidate)).thenReturn(Mono.just(saved));
			when(mapper.toResponse(saved)).thenReturn(out);
		...
		void nonProvince_parentMissing() {
			...
			// parent missing triggers error
			when(repo.existsById(eq(parent))).thenReturn(Mono.just(false));
			// duplicate check may also run in parallel; return anything
			when(repo.existsById(eq(code))).thenReturn(Mono.just(false));
			...
			verify(repo).existsById(parent);
			verify(repo).existsById(code);
			verify(repo, never()).save(any()); // thanks to Mono.defer(() -> repo.save(...))
		}
	...
	class Update {
		...
		void update_success() {
			...
			// Make mapper.update mutate the entity for the test
			doAnswer(inv -> {
